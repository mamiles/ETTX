...
<i18n:message id="blankTimeMsg" bundleRef="ettx_bundle" key="BlankTimeMsg" />
<i18n:message id="invalidLoginIDMsg" bundleRef="ettx_bundle" key="InvalidLoginIDMsg"/>
<i18n:message id="invalidIpAddressMsg" bundleRef="ettx_bundle" key="InvalidIpAddressMsg"/>
...
<script type="text/javascript">
function validateLeaseHistoryQuery(queryType) {
	var errorFound = false;
	var totalErrorMsg = "";
	var startTime = document.leaseHistoryQueryFB.startTime.value;
	var endTime = document.leaseHistoryQueryFB.endTime.value;

	if (startTime == "" ||
	    endTime == "") {
		totalErrorMsg = '<%= blankTimeMsg %>';
		errorFound = true;
	}

	if (queryType == "Subscriber") {
		var subscriber = document.leaseHistoryQueryFB.loginID.value;
		if (subscriber == "") {
			if (errorFound == true) {
				totalErrorMsg = totalErrorMsg + "  ";
			}
			totalErrorMsg = totalErrorMsg + '<%= invalidLoginIDMsg %>';
			errorFound = true;
		}
	} else if (queryType == "IpAddress") {
		var ipAddress = document.leaseHistoryQueryFB.ipAddress.value;
		if (ipAddress == "") {
			if (errorFound == true) {
				totalErrorMsg = totalErrorMsg + "  ";
			}
			totalErrorMsg = totalErrorMsg + '<%= invalidIpAddressMsg %>';
			errorFound = true;
		}
	} else {
		return true;
	...
	if (errorFound == true) {
		var msg = new uiiNotification ("ERROR", totalErrorMsg);
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="5"  BGCOLOR="E1E1E1" align="center">
  <tr> 
    <td width="100%" align="center" valign="middle"> <uii:layerDialog id="leaseHistoryQuery"> <uii:form action="/leaseHistoryQuery.do"> 
        <!----------------------------------------------------------------------------------------
	...
	----------->
		...
		<b><i18n:message bundleRef="ettx_bundle" key="leaseHistoryQueryTitle"/></b>
		</td>
          </tr>
          <uii:layerOptions> 
          <!----------------------------------------------------------------------------------------
				...
				----------->
          <tr> 
            <td nowrap bgcolor="#cecfce" colspan="2"> &nbsp;<i18n:message bundleRef="ettx_bundle" key="leaseQuerySearchBy"/>: 
		...
		<uii:radio property="currentLayer" value="IpAddress" layerName="IpAddress" onclick="showLayer('IpAddress')" >
			<i18n:message bundleRef="ettx_bundle" key="leaseQueryIpAddress"/>
		</uii:radio>
	    </td>
          </tr>
          </uii:layerOptions>
          ...
          <uii:layerContents name="Subscriber"> 
            <tr> 
              <td nowrap bgcolor="#cecfce" align="right">&nbsp;<i18n:message bundleRef="ettx_bundle" key="leaseQueryLoginID"/>:</td>
              <td width="100%" bgcolor="#cecfce"><uii:text property="loginID" /> </td>
            </tr>
            <tr> 
              <td nowrap bgcolor="#cecfce" align="right"><i18n:message bundleRef="ettx_bundle" key="leaseQueryStartDateTime"/>:</td>
	    ...
	    <tr> 
			...
			<i18n:message bundleRef="ettx_bundle" key="leaseQueryEndDateTime"/>:</td>
              <td width="100%" bgcolor="#cecfce">
			<uii:date id="endDate">
                  ...
                  <tr> 
                    <td width="100%"> <uii:img page="/RSRC/en_US/images/shim.gif" width="20" height="5"/> </td>
                     <td nowrap> <uii:button submitValue="Submit"
					    onclick="return validateLeaseHistoryQuery('Subscriber');"	
		...
		<uii:layerContents name="IpAddress"> 
            <tr> 
              <td nowrap bgcolor="#cecfce" align="right">&nbsp;<i18n:message bundleRef="ettx_bundle" key="leaseQueryIpAddress"/>:</td>
              <td width="100%" bgcolor="#cecfce"><uii:ip allowBlankValue="true" property="ipAddress" /> </td>
            </tr>
            <tr> 
              <td nowrap bgcolor="#cecfce" align="right"><i18n:message bundleRef="ettx_bundle" key="leaseQueryStartDateTime"/>:</td>
              <td width="100%" bgcolor="#cecfce">	
			...
			<uii:time id="startTime" property="startTime" allowBlankValue="true" format="24" />
		</td>
            </tr>
	    <tr> 
              <td nowrap bgcolor="#cecfce" align="right">
			...
			<uii:time id="endTime" property="endTime" allowBlankValue="true" format="24" />
		</td>
            </tr>
            <tr> 
              <!----------------------------------------------------------------------------------------
              ...
              <td colspan="2" bgcolor="silver"> 
                <table cellspacing=3 cellpadding=0 border=0 width="100%">
                  <tr> 
                    <td width="100%"> <uii:img page="/RSRC/en_US/images/shim.gif" width="20" height="5"/> </td>
                    <td nowrap> <uii:button submitValue="Submit"
					    onclick="return validateLeaseHistoryQuery('IpAddress');"	
					    popupWidth="1000" popupHeight="800" popupFeatures="resizable=yes,scrollbars=yes" >
					<i18n:message bundleRef="ettx_bundle" key="submit"/>
				</uii:button> 
		     </td>
    ...
    <td valign="top" align="RIGHT"> <uii:instructions>
                <p> <i18n:message bundleRef="instruction" key="reports.leasehistory.query.p1"/></p>
                  </uii:instructions>
    </td>
  </tr>
</table>
